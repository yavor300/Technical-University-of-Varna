...
void mergesort(int sorted[], int n);
...
int main()
{
    ...
    else
    {
        ...
        {   
            ...
            int merge_sorted[NUMBERS_COUNT];
            int shake_sorted[NUMBERS_COUNT];
            int shell_sorted[NUMBERS_COUNT];

            cout << "Successfully stored all numbers in an array.\n\n";
            for (int i = 0; i < NUMBERS_COUNT; i++)
            {
                merge_sorted[i] = not_sorted[i];
                shake_sorted[i] = not_sorted[i];
                shell_sorted[i] = not_sorted[i];
            }
            mergesort(merge_sorted, NUMBERS_COUNT);
            cout << "MERGE SORT: ";
            for (int i = 0; i < NUMBERS_COUNT; i++)
            {
                cout << merge_sorted[i] << " ";
            ...
            cout << "\n\n";
            shakesort(shake_sorted, NUMBERS_COUNT);
            cout << "SHAKE SORT: ";
            for (int i = 0; i < NUMBERS_COUNT; i++)
            {
                cout << shake_sorted[i] << " ";
...
void mergesort(int sorted[], int n)
{
    ...
    mergesort(sorted, nleft);
    mergesort(sorted + nleft, nright);
    int* p = new int[n];
