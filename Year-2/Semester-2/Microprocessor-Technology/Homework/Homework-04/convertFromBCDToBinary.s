...
@ която да връща в регистър R0 двоичния еквивалент на подадено пак
@ там (в R0) двуразрядно пакетирано BCD число.

.global convertFromBCDToBinary
convertFromBCDToBinary:
	MOV R0,#0b10010110		@ Запазване на BCD 8-битово пакетирано число
	AND R1,R0,#0b00001111	@ Запазване на младшите 4 бита от пакетираното BCD число
	MOV R2,R0,LSR#4			@ Запазване на старшите 4 бита от пакетираното BCD число
	MOV R3,R2,LSL#1			@ Умножаваме по 2 старшата част
	MOV R4,R2,LSL#3			@ Умножаваме по 8 старшата част
	ADD R0,R3,R4			@ Събираме младшите битове към умножената по 2 старша част
	ADD R0,R0,R1			@ Получаваме резултата в R0
	BX LR
...
@	mov r3,r7 
	...
	bl	convertFromBCDToBinary		@ coll user subroutine
