@ Да се напише подпрограма, която да връща в регистър R0 фактуриела на подаденото в R0 число.

.global Factorial
Factorial:
	MOV R1,#1			@ инициализираме резултат
	CMP R0,#0			@ сравняваме дали R0 е 0
	BEQ EndFactorial	@ ако е 0, приключваме изпълнението на програмата
CalculateFactorial:		@ branch за цикъл
	CMP R0,#1			@ проверка дали R0 е 1 за излизане от цикъла
	BEQ EndFactorial	@ излизане от цикъла
	MUL R1,R0,R1		@ натрупване на резултата чрез умножение
	SUB R0,R0,#1		@ намаляване на брояча
	B CalculateFactorial
...
@	mov r3,r7 
	...
	bl	Factorial		@ coll user subroutine
