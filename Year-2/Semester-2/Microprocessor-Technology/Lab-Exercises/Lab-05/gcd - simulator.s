@ Да се напише подпрограма на асемблерен език за намиране и записване  
@ в R0 най-големия общ делител на числата в R0 и R1
@ (алгоритъм - Евклид от Александрия).

.global GCD
GCD:
	ORRS R3,R0,R1		@ a == b && a == 0
	MOVEQ R0,#1			@ if a = b = 0 -> GCD(a,b) = 1
	CMPNE R1,#0 		@ if a != b -> b ?= 0
	BXEQ LR				@ if a = b = 0 -> end (GCD(a,b) = 1 or = a)
	CMPNE R0,#0			@ a == 0
	MOVEQ R0,R1			@ if a = 0 -> GCD(a,b) = b
	L1: SUBS R3,R0,R1	@ a - b
		SUBLO R1,R1,R0	@ if a < b -> b = b - a
		SUBHI R0,R0,R1	@ if a > b -> a = a - b
		BNE L1			@ if a <> b -> loop
	BX LR				@ if a = b -> end (GCD(a,b) = a)
