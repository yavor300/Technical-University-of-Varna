...
nsamebit:
	...
	MOV R1, #0b1010
	MOV R2, #0b1010
	EOR R1,R1,R0 		@ R1 = R1? R0 @ съвпадащите битове стават 0, несъвпадащите битове ставaт 1
	EOR R2,R2,R0		@ R2 = R2 ? R0 
	ORR R1,R1,R2		@ R1 = R1 ? R0 | R2 ? R0 
	MOV R2,#32			@ ще проверим 32 бита
	MOV R0,#0			@ Начална стойност на брояч на нули = 0;
LOOP: MOVS R1,R1,LSR #1	@ Изведи поредния бит в бит “Carry” на CPSR;
	ADDCC R0,R0,#1		@ Преброй поредния бит като съвпадащ, ако е 0;
	SUB	R2,R2,#1		@ Намаляваме брояча с 1;
	CMP R2,#0
	BHI LOOP	@ Ако брояча не е 0 - повтори;
	MOV R2,#32
	SUB R2,R2,R0
	MOV R1,#4
	SUB R0,R1,R2
...
@	mov r3,r7 
	...
	bl	nsamebit		@ coll user subroutine
