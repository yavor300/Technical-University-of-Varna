...
bitrev64:
	MOV R0,#0b1011
	MOV R2,#32			@ R2 ще е брояч за 32 измествания 
	LOOP: MOVS R0,R0,LSL#1	@ изместваме наляво R0 и запомняме флага "c"
	MOVS R1,R1,ROR#0	@ изместваме циклично надясно с използване на флаг "c", ROR#0 е еквивалентно на RRX
	ADC R0,R0,#0		@ Добавяме запомнения в CPSR флаг "c" в най-младшия бит на R0
	SUB R2,R2,#1		@ намаляваме брояча
	BNE LOOP			@ Ако не е 0 - повтаряме
...
@	mov r3,r7 
	...
	bl	bitrev64		@ coll user subroutine
