...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		case IDM_A3:
			{
				...
				if(res & MF_CHECKED)
					CheckMenuItem(hMenu, IDM_A4, MF_BYCOMMAND | MF_UNCHECKED);
				else
					CheckMenuItem(hMenu, IDM_A4, MF_BYCOMMAND | MF_CHECKED);

				break;
			}
		case IDM_A4: {
			HMENU hMenu = GetMenu(hWnd);
			UINT res = GetMenuState(hMenu, IDM_A7, MF_BYCOMMAND);

			if(res & MF_GRAYED)
				EnableMenuItem(hMenu, IDM_A7, MF_BYCOMMAND | MF_ENABLED);
			else
				EnableMenuItem(hMenu, IDM_A7, MF_BYCOMMAND | MF_GRAYED);
								 }
								 break;
		case IDM_A5:
		...
		case IDM_A9:
			...
			if(GetMenuState(hMenu, IDM_A9 + 1, MF_BYCOMMAND) == -1) // or disable
			{
				MENUITEMINFO mii;
