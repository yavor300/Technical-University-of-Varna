...
INT_PTR CALLBACK Task(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		else if (LOWORD(wParam) == IDC_BUTTON1) 
		{
			...
			int firstNumber = GetDlgItemInt(hDlg,IDC_EDIT1,LPt, SIG);
			int secondNumber = GetDlgItemInt(hDlg,IDC_EDIT2,LPt, SIG);

			if (IsDlgButtonChecked(hDlg,IDC_CHECK1) && IsDlgButtonChecked(hDlg,IDC_CHECK2)) {
				sum=firstNumber+secondNumber*secondNumber;
			...
			SetDlgItemInt(hDlg,IDC_EDIT3, sum,TRUE);
		}
		else if (LOWORD(wParam) == IDC_BUTTON2) 
		{
			...
			{
				GetDlgItemText(hDlg,IDC_EDIT4,inputStringNumbers,MAX_LOADSTRING);
				firstNumber=strtod(inputStringNumbers,&stopstring);
				GetDlgItemText(hDlg,IDC_EDIT5,inputStringNumbers,MAX_LOADSTRING);
				secondNumber=strtod(inputStringNumbers,&stopstring);
				if (secondNumber == 0) {
					MessageBox(hDlg,"Divisor cannot be 0!", "ERROR!", MB_OK | MB_ICONERROR);
				} else {
					sum = (firstNumber + secondNumber * secondNumber) / secondNumber;
				}
				sprintf_s(inputStringNumbers, "%8.3f",sum);
				SetDlgItemText(hDlg,IDC_EDIT6,inputStringNumbers);
			}
			else if (IsDlgButtonChecked(hDlg, IDC_RADIO2))
			{
				...
				if (firstNumber == 1) {
					...
					sum = secondNumber / (firstNumber - 1);
				}
				sprintf_s(inputStringNumbers, "%8.3f", sum);
				SetDlgItemText(hDlg,IDC_EDIT6,inputStringNumbers);
			}
