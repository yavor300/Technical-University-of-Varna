...
void del_element(tree *t, item_type item) {
    ...
    if (element->left == NULL && element->right == NULL) {  // No children
        ...
        del_element(right_min, right_min->item);  // Recursively delete the replacement node
    }
}
...
void add_from_file(tree **root, const char *filename) {
...
int main_(void) {
    ...
    free_tree(root);  // Free the memory used by the tree
    getchar();
    return EXIT_SUCCESS;
}
